...
import { Annex } from "./module/types/documentservice/annex"
import { Contract } from "./module/types/documentservice/contract"
import { Params } from "./module/types/documentservice/params"


export { Annex, Contract, Params };
...
export default {
	...
	getters: {
			...
			return state.Contract[JSON.stringify(params)] ?? {}
				...
				getContractAll: (state) => (params = { params: {}}) => {
					if (!(<any> params).query) {
						(<any> params).query=null
					}
			return state.ContractAll[JSON.stringify(params)] ?? {}
		},
				getAnnex: (state) => (params = { params: {}}) => {
					if (!(<any> params).query) {
						(<any> params).query=null
					}
			return state.Annex[JSON.stringify(params)] ?? {}
		},
				getAnnexAll: (state) => (params = { params: {}}) => {
					if (!(<any> params).query) {
						(<any> params).query=null
					}
	...
	actions: {
		init({ dispatch, rootGetters }) {
			console.log('Vuex module: cosmonaut.documentservice.documentservice initialized!')
		...
		async QueryContract({ commit, rootGetters, getters }, { options: { subscribe, all} = { subscribe:false, all:false}, params, query=null }) {
			try {
				const key = params ?? {};
				const queryClient=await initQueryClient(rootGetters)
				let value= (await queryClient.queryContract( key.id)).data
		...
		async QueryContractAll({ commit, rootGetters, getters }, { options: { subscribe, all} = { subscribe:false, all:false}, params, query=null }) {
			try {
				const key = params ?? {};
				const queryClient=await initQueryClient(rootGetters)
				let value= (await queryClient.queryContractAll(query)).data
				
					
				while (all && (<any> value).pagination && (<any> value).pagination.next_key!=null) {
					let next_values=(await queryClient.queryContractAll({...query, 'pagination.key':(<any> value).pagination.next_key})).data
					value = mergeResults(value, next_values);
				}
				commit('QUERY', { query: 'ContractAll', key: { params: {...key}, query}, value })
				if (subscribe) commit('SUBSCRIBE', { action: 'QueryContractAll', payload: { options: { all }, params: {...key},query }})
				return getters['getContractAll']( { params: {...key}, query}) ?? {}
			} catch (e) {
				throw new Error('QueryClient:QueryContractAll API Node Unavailable. Could not perform query: ' + e.message)
		...
		async QueryAnnex({ commit, rootGetters, getters }, { options: { subscribe, all} = { subscribe:false, all:false}, params, query=null }) {
			try {
				const key = params ?? {};
				const queryClient=await initQueryClient(rootGetters)
				let value= (await queryClient.queryAnnex( key.id)).data
				
					
				commit('QUERY', { query: 'Annex', key: { params: {...key}, query}, value })
				if (subscribe) commit('SUBSCRIBE', { action: 'QueryAnnex', payload: { options: { all }, params: {...key},query }})
				return getters['getAnnex']( { params: {...key}, query}) ?? {}
			} catch (e) {
				throw new Error('QueryClient:QueryAnnex API Node Unavailable. Could not perform query: ' + e.message)
		...
		async QueryAnnexAll({ commit, rootGetters, getters }, { options: { subscribe, all} = { subscribe:false, all:false}, params, query=null }) {
			try {
				const key = params ?? {};
				const queryClient=await initQueryClient(rootGetters)
				let value= (await queryClient.queryAnnexAll(query)).data
				
					
				while (all && (<any> value).pagination && (<any> value).pagination.next_key!=null) {
					let next_values=(await queryClient.queryAnnexAll({...query, 'pagination.key':(<any> value).pagination.next_key})).data
					value = mergeResults(value, next_values);
				}
				commit('QUERY', { query: 'AnnexAll', key: { params: {...key}, query}, value })
				if (subscribe) commit('SUBSCRIBE', { action: 'QueryAnnexAll', payload: { options: { all }, params: {...key},query }})
				return getters['getAnnexAll']( { params: {...key}, query}) ?? {}
			} catch (e) {
				throw new Error('QueryClient:QueryAnnexAll API Node Unavailable. Could not perform query: ' + e.message)
	...
	gas: "200000" }, memo})
		...
		async sendMsgSignAnnex({ rootGetters }, { value, fee = [], memo = '' }) {
			try {
				const txClient=await initTxClient(rootGetters)
	...
	gas: "200000" }, memo})
		...
		async sendMsgCreateAnnex({ rootGetters }, { value, fee = [], memo = '' }) {
			try {
				const txClient=await initTxClient(rootGetters)
	...
	gas: "200000" }, memo})
		...
		async MsgSignAnnex({ rootGetters }, { value }) {
			try {
				const txClient=await initTxClient(rootGetters)
				const msg = await txClient.msgSignAnnex(value)
				return msg
				...
				if (e == MissingWalletError) {
					...
					throw new Error('TxClient:MsgSignAnnex:Create Could not create message: ' + e.message)
				}
			}
		},
		...
		async MsgCreateAnnex({ rootGetters }, { value }) {
			try {
				const txClient=await initTxClient(rootGetters)
				const msg = await txClient.msgCreateAnnex(value)
				return msg
				...
				if (e == MissingWalletError) {
					...
					throw new Error('TxClient:MsgCreateAnnex:Create Could not create message: ' + e.message)
				}
			}
		},
